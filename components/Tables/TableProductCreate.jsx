    ...
    import { get, set, update, ref, remove, onValue } from "firebase/database";
    ...
    function TableProductCreate() {
        ...
        useEffect(() => {
            const dbref = ref(database, "Paket");
        ...
        let InsertData = () => {
            ...
            const paketRef = ref(database, `Paket/${namapaket}`);
            get(paketRef).then((snapshot) => {
                if (snapshot.exists()) {
                    alert("Paket sudah ada, gunakan nama yang berbeda");
                } else {
                    const deskripsiList = deskripsi.split('\n');// Konversi deskripsi menjadi list
                    // Create a reference to the Firebase Storage location
                    const storagePaketRef = storageRef(storage, `paket/${namapaket}`);

                    // Upload the file to Firebase Storage
                    const file = document.querySelector('input[type="file"]').files[0];
                    if (file) {
                        uploadBytes(storagePaketRef, file).then((snapshot) => {
                            console.log("File uploaded!", snapshot);

                            // Get the download URL for the uploaded file
                            getDownloadURL(storagePaketRef).then((downloadURL) => {
                                // Save the data including the download URL to the Realtime Database
                                set(paketRef, {
                                    deskripsi: deskripsi,
                                    harga: harga,
        ...
        let UpdateData = () => {
            if (isNullOrWhiteSpaces(namapaket)) {
                alert("Harap isi nama paket");
                return;
            }
        ...
        let DeleteData = (packageName) => {
            if (isNullOrWhiteSpaces(packageName)) {
                alert("Harap isi nama paket");
                return;
            }
        
            const paketRef = ref(database, `Paket/${packageName}`);
            get(paketRef).then((snapshot) => {
                if (snapshot.exists()) {
                    remove(paketRef)
                        .then(() => {
                            ...
                            >
                                ...
                                <div className="col-span-1 flex items-center">
                                    <div className="h-12.5 w-15 rounded-md">
                                        <img
                                            src={paket.imageUrl || "/placeholder-image.jpg"}
                                            width={60}
                                            height={50}
                                            alt="Product"
                                        />
                                ...
                                <div className="col-span-1 flex items-center">
                                    <p className="text-sm text-meta-3">{paket.harga}</p>
                                </div>
                ...
                <div className="grid grid-cols-1 gap-9 sm:grid-cols-2">
                    <div className="flex flex-col gap-9">
                        <div>
                            ...
                            <input
                                value={namapaket} onChange={e => setNamaPaket(e.target.value)}
                                type="text"
    ...
    export default TableProductCreate